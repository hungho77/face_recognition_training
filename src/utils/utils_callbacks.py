...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            ...
            self.summary_writer.log({'XNorm': xnorm})
            self.summary_writer.log({'Accuracy': acc2})
            self.summary_writer.log({'std2': std2})
            ...
            self.summary_writer.log({'Accuracy-Highest': self.highest_acc_list[i]})
            results.append(acc2)
...
#             path = os.path.join(data_dir, name + ".bin")
            if os.path.exists(data_dir):
                data_set = verification.load_dir(data_dir, image_size)
        ...
        if self.rank == 0 and global_step > 0 and global_step % self.frequent == 0:
            if self.init:
                ...
                if self.writer is not None:
                    # self.writer.log({'time_for_end': time_for_end})
                    self.writer.log({'learning_rate': learning_rate})
                    self.writer.log({'loss': loss.avg})
                if fp16:
