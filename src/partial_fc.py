...
class PartialFC(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(self.fp16):
            if self.margin_softmax._get_name() in ["ArcFace", "CosFace", "CombinedMarginLoss"]:
                norm_embeddings = normalize(embeddings)
                norm_weight_activated = normalize(weight)
                logits = linear(norm_embeddings, norm_weight_activated)
            elif self.margin_softmax._get_name() in ["AdaFace"]:
                norms = torch.norm(embeddings, 2, 1, True)
                norm_embeddings = torch.div(embeddings, norms)
                norm_weight_activated = normalize(weight)
                logits = linear(norm_embeddings, norm_weight_activated)
        ...
        if self.margin_softmax._get_name() in ["ArcFace", "CosFace", "CombinedMarginLoss"]:
            ...
            safe_norms.clone().detach()
            logits = self.margin_softmax(logits, safe_norms, labels)
            loss = self.dist_cross_entropy(logits, labels)
